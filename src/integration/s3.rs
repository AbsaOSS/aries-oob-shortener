...
impl S3Client {
    ...
    pub async fn get_object(&self, bucket: &str, object: &str, path: &str) -> SResult<()> {
        ...
        let bytes = response
            ...
            .collect()
            .await
            .map_err(|err| SError::from_msg(SErrorType::IOError, &format!("Failed to load response byte stream, error: {}", err)))?
            .into_bytes();
...
mod tests {
    ...
    async fn get_s3_data() {
        init_logger(false, None, None).unwrap();
        let mut tmp_dir = std::env::temp_dir();
        tmp_dir.push("testfile.txt");
        S3Client::new(S3ClientConfig { region: None })
            .await
            .unwrap()
            .get_object("ctoblockchaindev-euw1-dev-dlt-localhost", "testread/testfile.txt", tmp_dir.to_str().unwrap())
            .await
            .unwrap();
