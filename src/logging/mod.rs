...
pub fn init_logger(default_fields: Option<HashMap<String, Value>>, name: Option<&str>) -> SResult<()> {
    if log_json() {
        let subscriber_name = name.unwrap_or("dlt-shortener").to_string();
        let subscriber = subscriber::get_subscriber_json(subscriber_name, default_fields.unwrap_or_default());
